...
3. Install the packages `dplyr`, `ggplot2`, `tidyr`, `stringr`, `readr`, `foreign`, `readxl`, `haven`, `sandwich`, `prettyR`, and `lmtest`. Hint: You can also use the `Packages` Tab of RStudio to install and activate the libraries.
```{r,eval=FALSE}
...
6. Use the  command _setwd("c:/path")_ or _setwd(choose.dir())_ (only available on windows) to change the working directory to drive *c:* and path */path*. Note that the path name is structured by slashes (*/*), **not** backslashes (*\\*). Change the working directory and check if the change has been successful. Hint: The working directory can also be changed via the menu: _Session -- Set Working Directory_ or in the `Files` tab under `More`.
```{r,eval=FALSE}
setwd("/home/wmutschl/Documents/RKurs")
...
9. Why should you NOT use $c$ as a variable?
```{r}
...
`x^y`, `x^(1/y)`, `log(exp(y))`, `y*c(-1,1)`, `x+c(-1,0,1)`, and `sum(y*c(-1,1),na.rm=TRUE)`.
```{r}
x^y
x^(1/y)
log(exp(y))
...
8. Predict what the following commands will return: `cbind(1,X)`, `rbind(Y,c(1,2,3))`, `X%*%I`, `dim(X%*%Y)`, `t(Y)+Z`, `solve(t(Z)%*%Z)%*%(t(Z)%*%Z)`.
```{r}
cbind(1,X)
rbind(Y,c(1,2,3))
X%*%I
...
rm(list = ls())
```
1. Load `dplyr`, `ggplot2` and `gapminder` package which will provide you with the **gapminder** dataset.
```{r}
library("dplyr")
library("ggplot2")
library("gapminder")
...
4. Now add a log scale with `scale_x_log10` for gdpPercap to your ggplot scatterplot.
```{r}
ggplot(gapminder_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_point() +
...
14. Now let's animate the plot using `gganimate`. Have a look at the following code:
```{r}
library(gganimate)
...
rm(list = ls())
```
In 1973, Francis Anscombe famously created four datasets with remarkably similar numerical properties, but obviously different graphic relationships. The `anscombe.csv` dataset contains the x and y coordinates for these four datasets, along with a grouping variable, `dataset`, that distinguishes the quartet.

1. Load the dataset into the variable `anscombe`
```{r}
library(readr)
anscombe <- read_csv("data/anscombe.csv")
```

2. For each of the four sets of data points, compute the mean, standard deviation and correlation of x and y.
...
3. Create a scatterplot for each dataset into a single plot and add a regression line to it with `geom_smooth`. Color the points of the different datasets accordingly and use `scale_color_brewer(palette="Set1")` and `theme_light()`. Save the plot as `anscombe_scatter1` and print it.
```{r}
library("ggplot2")
my.formula <- y ~ x
anscombe_scatter1 <- ggplot(data = anscombe, aes(x = x, y = y, col=dataset)) +
  geom_point() +
  geom_smooth(method = "lm", formula = my.formula, se=FALSE) +
  scale_color_brewer(palette="Set1") +
  theme_light()
print(anscombe_scatter1)
```

4. Note that this scatterplot is not a good visualization. Therefore, we use the concept of _Small Multiples_ and create a scatterplot for each dataset separetly. Save this plot as `anscombe_scatter2` and print it.
```{r}
library("ggplot2")
my.formula <- y ~ x
anscombe_scatter2 <- anscombe_scatter1 +
  facet_wrap(~ dataset)
print(anscombe_scatter2)
```

4. Load the package `ggpmisc` and add the following code to your scatterplot:
...
```{r}
glimpse(weather5) # View the structure of weather5
...
glimpse(weather6) # Look at result
```
...
***
# Exploratory Data Analysis of UN Votes
```{r include=FALSE}
rm(list = ls())
```

We will consider the historical voting of the United Nations General Assembly, including analyzing differences in voting between countries, across time, and among international issues. Note that this exercise is based on the same course on datacamp.com.

1. Load the packages `ggplot2`, `tidyr`, `dplyr`, and `countrycode`. Import the data using `load("un_votes.RData")` and have a look at `un_votes`:

- each row is a country-vote pair
- `rcid` is the roll call ID
- `session` is the year; note that the UN started voting in 1946, and holds one session per year
- `vote` is the vote, where 1=yes, 2=abstain, 3=no, 8=not present, 9=not a member 
- `ccode` are encoded Correlates of War codes. 
```{r}
library("ggplot2");library("tidyr"); library("dplyr"); library("countrycode")
load("data/un_votes.RData")
```

2. Now we will preprocess our dataset. Create a new dataset from `un_votes` that

- focuses only on actual votes, i.e. get rid of observations where countries were not present or not a member.
- adds a new variable called `year`
- translates the country codes into human readable form

Hint: You can use the `countrycode` package to translate them. That is, to convert `ccode=2` to a human readable form, use `countrycode(2, "cown", "country.name")` which gets you "United States".

```{r}
# Load the countrycode package
library(countrycode)
countrycode(2, "cown", "country.name")
votes_processed <- un_votes %>%
  filter(vote <= 3) %>%
...
2. Now we will focus on the percentage of Yes votes as a metric for the "agreeableness" of countries. Summarize your data by computing the

- total number of votes (hint: use `n()`)
- fraction of "yes" votes (hint: use `mean(vote == 1)`)
...
3. Now summarize voting patterns between countries and save this dataset as `by_country`. Which countries voted "yes" least often and which ones voted "yes" most often?
```{r}
...
by_country
by_country %>%
  arrange(percent_yes) #note that Zanzibar has only 2 votes! Always look at totals!
by_country %>%
  arrange(desc(percent_yes))
```

4. In the last exercise, you may have noticed that the country that voted least frequently, Zanzibar, had only 2 votes in the entire dataset. You certainly can't make any substantial conclusions based on that data! Therefore,  use filter() to remove countries that have fewer than 100 votes.
```{r}
by_country %>%
  arrange(percent_yes) %>%
  filter(total >= 100)
...
5. How much does the average "agreeableness" of countries change from year to year? To examine this, you can use `group_by()` to perform your summary not for the entire dataset, but within each year. Save this dataset as `by_year`. Plot the percentage of "yes" votes over time and try to visualize the trend.
```{r}
...
6. Now summarize by both year and country, constructing a dataset `by_year_country` that shows what fraction of the time each country votes "yes" in each year. Plot the trend for a particular country of your choice.
```{r}
...
DE_by_year <- by_year_country %>%
  filter(country == "Germany")

# Line plot of percent_yes over time for Germany only
ggplot(DE_by_year, aes(year, percent_yes)) +
  geom_point() +
  geom_smooth()
```

7. Now compare voting trends for the United States, the UK, France, Germany, Italy and India. Create a filtered version of `by_year_country` called `filtered_4_countries` with the four countries (you may find the `%in%` operator useful here). Show the trend for each of these countries on the same graph, using color to distinguish each country. Save this plot as `ggp_4_countries`.

```{r}
# Vector of four countries to examine
countries <- c("United States", "United Kingdom", "France", "Germany", "Italy", "India")
...
ggp_4_countries <- ggplot(filtered_4_countries, aes(year, percent_yes, color = country)) +
  geom_point() + 
  geom_smooth(se=FALSE)
print(ggp_4_countries)
```

8. Note that this plot is too crowded, and it is advisable to facet, i.e. giving each country its own sub-plot. To do so, add a `facet_wrap()` to `ggp_4_countries`. Try if you liked `scales= "free_y"` as an option to `facet_wrap()`.
...
9. Let's quantify the trend from a linear regression, first for one country, e.g. use your dataset `DE_by_year`. Run a linear regression predicting `percent_yes` from `year` and store the model object into `DE_fit`. Call summary on this object. What's the estimated change each year of the probability of the US voting "yes"? What is the p-value of the relationship between year and percent_yes?

```{r}
DE_fit <- lm(percent_yes ~ year, data=DE_by_year)
summary(DE_fit)
```
...
1. Let's consider a simple count data example. Let
$$ X \sim N (0,1) $$
+  Compute the probability $P(|X|>3.5)$.
```{r}
2 * (1 - pnorm(3.5))
```
